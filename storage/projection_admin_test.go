...
func (s *StorageSuite) TestRebuildProjection() {
	...
	s.Require().NoError(s.storage.SetScore(ctx, alice, testBoardId, 50, "a5", ""))
	s.Require().NoError(s.storage.IncrementScore(ctx, alice, testBoardId, 6, "a6", ""))

	bob := player.GenerateID()
	_, err = s.storage.CreatePlayerProfile(ctx, &player.Profile{PlayerId: bob, PlayerName: "bob"}, "")
	s.Require().NoError(err)
	s.Require().NoError(s.storage.SetScore(ctx, bob, testBoardId, 42, "b2", ""))

	// wipe the projection, then rebuild
	s.Require().NoError(s.rawClient.Del(ctx, leaderboardKey(testBoardId)).Err())
	s.Require().NoError(s.storage.RebuildProjection(ctx, testBoardId))

	aliceScore, err := s.rawClient.ZScore(ctx, leaderboardKey(testBoardId), string(alice)).Result()
	s.Require().NoError(err)
	s.Require().Equal(56.0, aliceScore)

	bobScore, err := s.rawClient.ZScore(ctx, leaderboardKey(testBoardId), string(bob)).Result()
	s.Require().NoError(err)
...
func (s *StorageSuite) TestRebuildProjectionOnlyAffectsRequestedBoard() {
	...
	s.Require().NoError(
		s.rawClient.ZAdd(ctx, leaderboardKey(testBoardId), redis.Z{Score: 999, Member: string(playerId)}).Err(),
	)
	s.Require().NoError(s.rawClient.Del(ctx, leaderboardKey("weekly")).Err())

	s.Require().NoError(s.storage.RebuildProjection(ctx, "weekly"))

	mainScore, err := s.rawClient.ZScore(ctx, leaderboardKey(testBoardId), string(playerId)).Result()
	s.Require().NoError(err)
...
func (s *StorageSuite) TestVerifyProjection() {
	...
	s.Require().NoError(s.storage.SetScore(ctx, playerId, testBoardId, 34, "v0", ""))
	s.Require().NoError(s.storage.IncrementScore(ctx, playerId, testBoardId, 6, "v1", ""))

	// consistent: every write went through the script
	mismatches, err := s.storage.VerifyProjection(ctx, testBoardId)
	s.Require().NoError(err)
	s.Require().Empty(mismatches)

	exists, err := s.rawClient.Exists(ctx, boardVerifyKey(testBoardId)).Result()
	s.Require().NoError(err)
	s.Require().Equal(int64(0), exists)

	// corrupt the projection directly -> drift is detected
	s.Require().NoError(s.rawClient.ZIncrBy(ctx, leaderboardKey(testBoardId), 1000, string(playerId)).Err())
	mismatches, err = s.storage.VerifyProjection(ctx, testBoardId)
	s.Require().NoError(err)
	s.Require().Len(mismatches, 1)
	s.Require().Equal(string(testBoardId), mismatches[0].BoardID)
...
func (s *StorageSuite) TestVerifyProjectionOnlyChecksRequestedBoard() {
	...
	playerId := s.createPlayer("bob")
	s.createBoard("weekly", "Weekly", mockedTime)
	s.Require().NoError(s.storage.SetScore(ctx, playerId, testBoardId, 10, "v-m", ""))
