...
```
XADD score:events * type set player_id alice amount 0 request_id r1
```
...
- `XLEN score:events` — number of entries.
- `XREVRANGE score:events + - COUNT 10` — newest first (history endpoint).
- `XRANGE score:events <last_seen_id> +` — resume reading from a checkpoint
...
`leaderboard`, the ledger stream `score:events`, and the idempotency hash
`score:applied`; the returned triple is `{ applied, new_score, stream_id }`.

```
EVAL "<script>" 3 leaderboard score:events score:applied set       alice  0  r1
→ 1) 1   2) "0"   3) "1752771000001-0"
EVAL "<script>" 3 leaderboard score:events score:applied increment alice  3  r2
→ 1) 1   2) "3"   3) "1752771000002-0"
EVAL "<script>" 3 leaderboard score:events score:applied increment alice 10  r3
→ 1) 1   2) "13"  3) "1752771000003-0"
EVAL "<script>" 3 leaderboard score:events score:applied set       alice 50  r4
→ 1) 1   2) "50"  3) "1752771000004-0"
EVAL "<script>" 3 leaderboard score:events score:applied increment alice -4  r5
...
XLEN score:events             → 5          (ledger: full history)
```
...
```
XREVRANGE score:events + - COUNT 3
